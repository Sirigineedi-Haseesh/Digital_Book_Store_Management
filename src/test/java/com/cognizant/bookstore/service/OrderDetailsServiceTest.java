...
class OrderDetailsServiceTest {
    ...
    void testChangeOrderStatus_Success() {
        ...
        when(orderRepository.save(any(OrderDetails.class))).thenReturn(order);

        String result = orderDetailsService.changeOrderStatus(orderId, status);

        assertEquals("Order status updated successfully to: " + status, result);
    }

    // Test changeOrderStatus with order not found
    @Test
    ...
    // Test changeOrderStatus with invalid status
    @Test
    void testChangeOrderStatus_InvalidStatus() {
        Long orderId = 1L;
        String invalidStatus = "INVALID_STATUS";

        when(orderRepository.findById(orderId)).thenReturn(Optional.of(new OrderDetails()));

        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            orderDetailsService.changeOrderStatus(orderId, invalidStatus);
        });

        assertEquals("Invalid status: " + invalidStatus, exception.getMessage());
    }
}
