...
public class ReviewAndRatingServiceImpl implements ReviewAndRatingService {
    ...
    @Override
    public List<ReviewAndRating> getReviewsByBookTitle(String bookTitle) {
        ...
        List<ReviewAndRating> reviews = reviewAndRatingRepository.findByBookBookId(bookId);

        if (reviews.isEmpty()) {
            throw new ReviewNotFoundException("No reviews found for book: " + bookTitle);
        }
